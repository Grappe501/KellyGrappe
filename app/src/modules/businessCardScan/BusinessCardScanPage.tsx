...
//
// ✅ Updated to match modules/intake/processIntake.ts adapter contract:
//    processIntake({ source, form })
...
function buildLocationLabel(p: { city?: string; county?: string; state?: string }): string | undefined {
  const city = safeTrim(p.city)
  const county = safeTrim(p.county)
  const state = safeTrim(p.state) || "AR"
...
export default function BusinessCardScanPage() {
  ...
  const [originNote, setOriginNote] = useState("Imported via Business Card Scanner.")

  const analyzeAbortRef = useRef<AbortController | null>(null)

  const isBusy = stage === "ANALYZING" || stage === "SAVING"
  const canAnalyze = !!frontFile && !isBusy
  const emailOk = isValidEmail(extracted.email)

  /* ================================
IMAGE HANDLING
================================ */

  const handleFront = useCallback(async (file: File | null) => {
    setError(null)
    setFrontFile(file)
    setFrontPreview("")
    if (!file) return
  ...
  const handleBack = useCallback(async (file: File | null) => {
    setError(null)
    setBackFile(file)
    setBackPreview("")
    if (!file) return
...
================================ */
  ...
  const analyze = useCallback(async () => {
    ...
    try {
      ...
      const front = frontPreview || (await fileToDataUrl(frontFile))

      const back = backFile
        ? (backPreview || (await fileToDataUrl(backFile)))
        : undefined
      ...
      if (!res.ok || !json.ok) {
        throw new Error(json.error || "Scan function failed")
      }
...
================================ */
  ...
  const save = useCallback(async () => {
    ...
    if (!hasCore) {
      setError("Must have name, email, or phone")
      return
    }
    ...
    try {
      ...
      // ✅ Adapter contract: { source, form }
      // We pass a "form-like" payload and let modules/intake/processIntake.ts map it to the pipeline.
      await processIntake({
        source: "BUSINESS_CARD_SCAN",
        form: {
          // identity
          fullName: safeTrim(extracted.fullName) || undefined,
          email: safeTrim(extracted.email).toLowerCase() || undefined,
          ...
          zip: safeTrim(extracted.zip) || undefined,

          // org/context
          organization: safeTrim(extracted.organization) || undefined,
          metWhere: "Business Card Scan",
          conversationNotes:
          ...
          // these are used by the pipeline follow-up snapshot (if/when supported)
          location: locationLabel,
          notes: safeTrim(extracted.notes) || undefined,
          permissionToContact: true,

          // facebook (kept as best-effort fields; if the Contact schema doesn’t include them,
          // the adapter/pipeline should ignore them or treat them as rawPayload only)
          ...
          // keep everything for audit/debug
          __raw: {
            originNote: safeTrim(originNote) || undefined,
            extracted,
          },
        },
      })
  ...
  }, [extracted, originNote, emailOk, isBusy, nav])
    ...
    <Container>
      <div className="space-y-6">
        <div className="rounded-3xl bg-gradient-to-r from-indigo-600 to-purple-600 p-6 text-white shadow-xl">
          <div className="text-xs font-bold tracking-wide">CAMPAIGN FIELD SYSTEM</div>
          <div className="text-2xl font-extrabold">Business Card Intake</div>
          <div className="text-sm opacity-90">Capture → AI → Review → Save</div>
        </div>

        <Card>
          <CardHeader title={stage} subtitle="Production Field Intake" />
              ...
              <>
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {frontPreview && (
                      <img src={frontPreview} className="mt-3 rounded-xl border" />
                    )}
                  </div>
                  ...
                  <div>
                    ...
                    <Input
                      ...
                      accept="image/*"
                      capture="environment"
                      onChange={(e) => handleBack(e.target.files?.[0] ?? null)}
                    />

                    {backPreview && (
                      <img src={backPreview} className="mt-3 rounded-xl border" />
                    )}
                  </div>
                </div>

                <div className="flex justify-between">
                  <Button variant="secondary" onClick={() => nav("/live-contact")}>
                    Back
                  </Button>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setExtracted((p) => ({ ...p, fullName: e.target.value }))
                      }
                    />
                  </div>
                  ...
                  <div>
                    ...
                    {!emailOk && <ErrorText>Invalid email</ErrorText>}
                  </div>
                </div>
